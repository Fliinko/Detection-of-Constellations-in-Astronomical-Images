...
def CompressTemplate(template):
    ...
    try:
        temp1 = cv2.imread(template, cv2.COLOR_RGB2GRAY)
        temp1 = cv2.resize(temp1, (800,800), interpolation = cv2.INTER_AREA)
        cv2.imwrite("PreprocessedTemplates/" + template+ ".png", temp1, [cv2.IMWRITE_PNG_COMPRESSION, 0])
...
for x in os.listdir("Matchable"):
    for y in os.listdir("Templates"):
        ...
        for i in method:
            ...
            Rounder(_maxVal)
            print("Matching", x, "to", y,":\tAccuracy: ", _maxVal)
